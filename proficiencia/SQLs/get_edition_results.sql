...
st.acronym UF,
st.name Estado,
st.ibge_id as 'IBGE ID Estado',
c.name Cidade,
c.ibge_id as 'IBGE ID Cidade',
d.name Dependência,
dis.name Matéria,
dpa.grade_id Ano,
...
IF(p.disclosure <> 1, ROUND(p.qualitative_3), ((ROUND(p.qualitative_3)*100)/p.with_proficiency_weight)) percent_avancado,
IF(p.disclosure <> 1, ROUND(p.qualitative_2), ((ROUND(p.qualitative_2)*100)/p.with_proficiency_weight)) percent_proficiente,
IF(p.disclosure <> 1, ROUND(p.qualitative_1), ((ROUND(p.qualitative_1)*100)/p.with_proficiency_weight)) percent_basico,
IF(p.disclosure <> 1, ROUND(p.qualitative_0), ((ROUND(p.qualitative_0)*100)/p.with_proficiency_weight)) percent_insuficiente,
IF(p.disclosure <> 1, ROUND(p.level_optimal), ((ROUND(p.qualitative_3)*100)/p.with_proficiency_weight) + ((ROUND(p.qualitative_2)*100)/p.with_proficiency_weight)) percent_possui_proficiencia,
p.average Média,
CASE p.disclosure 
