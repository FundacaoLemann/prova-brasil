...
INSERT INTO aggregation_students
	...
	l.level_qualitative AS level_qualitative
	FROM
	provabrasil_2015.alunos_9ef AS a
	LEFT JOIN waitress_entities.school AS sc ON sc.ibge_id=a.ID_ESCOLA 
	LEFT JOIN waitress_entities.city AS c ON c.ibge_id=a.ID_MUNICIPIO
	LEFT JOIN waitress_entities.state AS st ON st.ibge_id=a.ID_UF
	LEFT JOIN provabrasil_2015.escolas AS es ON es.ID_ESCOLA=a.ID_ESCOLA
	LEFT JOIN provabrasil_2015.level AS l ON l.grade_id=9 AND discipline_id=1 AND l.min <= a.PROFICIENCIA_LP_SAEB AND l.max > a.PROFICIENCIA_LP_SAEB
	WHERE a.IN_PROFICIENCIA=1;
...
INSERT INTO aggregation_students
	...
	LEFT JOIN waitress_entities.state AS st ON st.ibge_id=a.ID_UF 
	LEFT JOIN provabrasil_2015.escolas AS es ON es.ID_ESCOLA=a.ID_ESCOLA
	LEFT JOIN provabrasil_2015.level AS l ON l.grade_id=5 AND discipline_id=2 AND l.min <= a.PROFICIENCIA_MT_SAEB AND l.max > a.PROFICIENCIA_MT_SAEB
	WHERE a.IN_PROFICIENCIA=1;
...
INSERT INTO aggregation_students
	...
	LEFT JOIN waitress_entities.state AS st ON st.ibge_id=a.ID_UF
	LEFT JOIN provabrasil_2015.escolas AS es ON es.ID_ESCOLA=a.ID_ESCOLA
	LEFT JOIN provabrasil_2015.level AS l ON l.grade_id=9 AND discipline_id=2 AND l.min <= a.PROFICIENCIA_MT_SAEB AND l.max > a.PROFICIENCIA_MT_SAEB
	WHERE a.IN_PROFICIENCIA=1;
COMMIT;