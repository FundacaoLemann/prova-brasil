DELIMITER ;;
CREATE PROCEDURE `calc_survey_director_city_state_brasil`()
BEGIN
  ...
  DECLARE opcoes CURSOR FOR 
		select region.name as aggregation, dependence.name as dependence, column_name as 'column'
		 ...
		 where table_name='survey_director_responses_2015' and column_name like 'TX_RESP_Q%';
  ...
  loop_leitura: LOOP
	...
	set @queryQ = '
        ...
        select
          ...
          0 as sampling_error
        from survey_director_responses_2015 as sdr
           inner join question as q on q.order = :num and q.survey_id = 1 and q.edition_id = 6
