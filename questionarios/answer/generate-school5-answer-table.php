...
(SELECT 
    ...
    WHERE waitress_entities.state.ibge_id = {$data[2]}
) as 'state_id',
waitress_entities.city.id as 'city_id',
(SELECT 
    waitress_entities.school.id as 'school_id' FROM waitress_entities.school 
    WHERE waitress_entities.school.ibge_id = {$data[5]}
) as 'school_id',
{$data[6]},
{$data[7]},
{$data[34]},
...
WHERE 
waitress_entities.city.ibge_id = {$data[3]}
;";
