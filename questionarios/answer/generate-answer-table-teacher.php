...
while (($data = fgetcsv($handle, 5000, ",")) !== false) {
    ...
    // INSERT ROWS
    $answerInsertSql = "INSERT INTO `{$tableName}` SELECT (SELECT waitress_entities.state.id as 'state_id' FROM waitress_entities.state WHERE waitress_entities.state.ibge_id = {$data[1]}) as 'state_id', waitress_entities.city.id as 'city_id', {$data[4]}, {$data[8]}, {$data[7]}, '" . implode("','", $data) . "' FROM waitress_entities.city WHERE waitress_entities.city.ibge_id = {$data[2]};";
