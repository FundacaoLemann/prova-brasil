DELIMITER ;;
CREATE PROCEDURE `calc_survey_student_9_city_state_brasil`()
BEGIN
  ...
  DECLARE opcoes CURSOR FOR
        SELECT region.name AS aggregation, dependence.name AS dependence, column_name AS 'column'
         ...
         WHERE table_name='survey_students_responses_2015_9ano' AND column_name LIKE 'TX_RESP_Q%';
  ...
  loop_leitura: LOOP
    ...
    SET @queryQ = '
        ...
        SELECT
          ...
          SUM(:column = \'*\') AS erase,
          0 AS sampling_error
        FROM survey_students_responses_2015_9ano AS sdr
